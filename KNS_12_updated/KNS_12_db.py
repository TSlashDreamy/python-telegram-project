...
cur.executescript("""
CREATE TABLE if not exists KN_12_fri (
   NumPar INT  PRIMARY KEY NOT NULL,
   name TEXT NOT NULL,
   pochatokParu TIME NOT NULL
);""")

con.execute("INSERT or REPLACE INTO KN_12_fri(NumPar, name,pochatokParu) VALUES(2,'Економіка і бізнес лекція','10:20')");
con.execute("INSERT or REPLACE INTO KN_12_fri(NumPar, name,pochatokParu) VALUES(3,'Економіка і бізнес практичні','12:10')");
con.commit()
...
'\n'
print(cur.fetchall())
cur.execute("SELECT * from KN_12_fri")
print('################  ПЯТНИЦЯ  ################')
print(cur.fetchall())
